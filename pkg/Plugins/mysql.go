...
func MysqlScan(info *common2.HostInfo) (tmperr error) {
	if common2.IsBrute {
		return
	}
	starttime := time.Now().Unix()
	for _, user := range common2.Userdict["mysql"] {
		for _, pass := range common2.Passwords {
			...
			if flag == true && err == nil {
				...
				if time.Now().Unix()-starttime > (int64(len(common2.Userdict["mysql"])*len(common2.Passwords)) * common2.Timeout) {
					return err
				}
			}
		}
...
func MysqlConn(info *common2.HostInfo, user string, pass string) (flag bool, err error) {
	flag = false
	Host, Port, Username, Password := info.Host, info.Ports, user, pass
	dataSourceName := fmt.Sprintf("%v:%v@tcp(%v:%v)/mysql?charset=utf8&timeout=%v", Username, Password, Host, Port, time.Duration(common2.Timeout)*time.Second)
