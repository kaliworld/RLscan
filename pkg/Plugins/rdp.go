...
func RdpScan(info *common2.HostInfo) (tmperr error) {
	if common2.IsBrute {
		return
	}
	...
	var signal bool
	var num = 0
	var all = len(common2.Userdict["rdp"]) * len(common2.Passwords)
	var mutex sync.Mutex
	brlist := make(chan Brutelist)
	port, _ := strconv.Atoi(info.Ports)

	for i := 0; i < common2.BruteThread; i++ {
		wg.Add(1)
		go worker(info.Host, common2.Domain, port, &wg, brlist, &signal, &num, all, &mutex, common2.Timeout)
	}

	for _, user := range common2.Userdict["rdp"] {
...
func (g *Client) Login(domain, user, pwd string, timeout int64) error {
	conn, err := common2.WrapperTcpWithTimeout("tcp", g.Host, time.Duration(timeout)*time.Second)
	if err != nil {
