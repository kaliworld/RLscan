...
func WebTitle(info *common2.HostInfo) error {
	if common2.Scantype == "webpoc" {
		WebScan2.WebScan(info)
		return nil
	}
	err, CheckData := GOWebTitle(info)
	info.Infostr = WebScan2.InfoCheck(info.Url, &CheckData)

	if !common2.NoPoc && err == nil {
		WebScan2.WebScan(info)
	} else {
...
func GOWebTitle(info *common2.HostInfo) (err error, CheckData []WebScan2.CheckDatas) {
		...
		if ip.To4() != nil {
			if info.Url == "" {
				...
				default:
					...
					protocol := GetProtocol(host, common2.Timeout)
...
func geturl(info *common2.HostInfo, flag int, CheckData []WebScan2.CheckDatas) (error, string, []WebScan2.CheckDatas) {
	//flag 1 first try
	//flag 2 /favicon.ico
	//flag 3 302
	//flag 4 400 -> https
	...
	CheckData = append(CheckData, WebScan2.CheckDatas{body, fmt.Sprintf("%s", resp.Header)})
	var reurl string
	if flag != 2 {
		if !utf8.Valid(body) {
...
func GetProtocol(host string, Timeout int64) (protocol string) {
	...
	socksconn, err := common2.WrapperTcpWithTimeout("tcp", host, time.Duration(Timeout)*time.Second)
	if err != nil {
		return
	}
