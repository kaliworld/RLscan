...
func NetBIOS(info *common2.HostInfo) error {
...
func NetBIOS1(info *common2.HostInfo) (netbios NetBiosInfo, err error) {
	...
	defer conn.Close()
	err = conn.SetDeadline(time.Now().Add(time.Duration(common2.Timeout) * time.Second))
	if err != nil {
		return
	}
...
func GetNbnsname(info *common2.HostInfo) (netbios NetBiosInfo, err error) {
	...
	conn, err := net.DialTimeout("udp", realhost, time.Duration(common2.Timeout)*time.Second)
	if err != nil {
		return
	}
	defer conn.Close()
