...
func MS17010(info *common2.HostInfo) error {
	if common2.IsBrute {
		return nil
	}
...
func MS17010Scan(info *common2.HostInfo) error {
	ip := info.Host
	// connecting to a host in LAN if reachable should be very quick
	conn, err := common2.WrapperTcpWithTimeout("tcp", ip+":445", time.Duration(common2.Timeout)*time.Second)
	if err != nil {
