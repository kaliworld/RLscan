...
func Inithttp() {
	...
	if common2.WebTimeout == 0 {
		common2.WebTimeout = 5
	}
	err := InitHttpClient(common2.PocNum, common2.Proxy, time.Duration(common2.WebTimeout)*time.Second)
	if err != nil {
		log.Fatal(err)
	}
}
...
func InitHttpClient(ThreadsNum int, DownProxy string, Timeout time.Duration) error {
	...
	if common2.Socks5Proxy != "" {
		dialSocksProxy, err := common2.Socks5Dailer(dialer)
		if err != nil {
			return err
		}
